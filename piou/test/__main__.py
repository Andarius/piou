...
def foo_main(
    ...
    foo4: Literal["foo", "bar"] = Option(
        None, "--foo4", help="Foo4 argument", case_sensitive=False
    ),
    ...
    foo9: str = Option(
        None, "--foo9", help="Foo9 argument", choices=[f"item{i}" for i in range(4)]
    ),
...
def sub_bar_main(
    foo: Path = Option(Path("/tmp"), "--foo", help="Foo argument"), **kwargs
):
